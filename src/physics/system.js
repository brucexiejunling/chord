...
const Kernel = require('../core');

let ParticleSystem = function (repulsion, stiffness, friction, centerGravity, targetFps, dt, precision, integrator) {
  ...
  var that = {
    ...
    addEdge: function (source, target, data) {
      ...
      state.adjacency[dst] = state.adjacency[dst] || {}
      state.adjacency[dst][src] = state.adjacency[dst][src] || []

      var exists = (state.adjacency[src][dst].length > 0 || state.adjacency[dst][src].length > 0)
      if (exists) {
        // probably shouldn't allow multiple edges in same direction
        // between same nodes? for now just overwriting the data...
        if (state.adjacency[src][dst].length > 0) {
          state.adjacency[src][dst].data && Object.assign(state.adjacency[src][dst].data, edge.data)
        } else {
          state.adjacency[dst][src].data && Object.assign(state.adjacency[dst][src].data, edge.data)
        }
        return
      } else {
    ...
    toScreen: function (p) {
      ...
      // return new Point(Math.floor(sx), Math.floor(sy))
    ...
    nearestEdge: function (pos) {
      ...
      each(state.edges, function (id, edge) {
        //先计算出三个点对应的三条边的距离, s1(point到node1的距离), s2(point到node2的距离), s3(两个node之间的距离).
        // 余弦定理
        //cos(A2) = (s1^2 + s3^2 - s2^2)/2s1s3 = 邻边 / s1;
        //勾股定理 s1^2 = distance^2 + (s1 * cos(A2))^2
        // distance = Math.sqrt(s1^2 - (s1 * cos(A2))^2) = Math.sqrt(s1^2 - (s1^2 + s3^2 - s2^2)/2s3)^2);
        var node1 = edge.source,
          node2 = edge.target;
        var pt1 = node1.p,
          pt2 = node2.p;
        var s1 = that.toScreen(pt1).subtract(pos).magnitude(),
          s2 = that.toScreen(pt2).subtract(pos).magnitude();
        var s3 = that.toScreen(pt1).subtract(that.toScreen(pt2)).magnitude();
        if (s3 > s2 && s3 > s1) { //s3必须作为最长边
          var distance = Math.sqrt(s1 * s1 - ((s1 * s1 + s3 * s3 - s2 * s2) / 2 / s3) * ((s1 * s1 + s3 * s3 - s2 * s2) / 2 / s3))
          if ((min.distance === null || distance < min.distance) && distance) {
            min = {
              edge: edge,
              distance: distance
            };
