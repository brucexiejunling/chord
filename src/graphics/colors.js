...
let _isRGB = function (color) {
  if (!color || typeof color != 'object') return false
  let components = objkeys(color).sort().join("")
...
let Colors = {
  ...
  decode: function (clr) {
    ...
    if (typeof clr == 'string') {
      ...
      let nameMatch = that.CSS[clr.toLowerCase()]
      if (nameMatch !== undefined) {
        clr = nameMatch
      }
      let hexMatch = clr.match(iscolor_re)
      if (hexMatch) {
  ...
  mix: function (color1, color2, proportion) {
    let c1 = that.decode(color1)
    let c2 = that.decode(color2)

    // let mixed = ... should this be a triplet or a string?
  },
  blend: function (rgbOrHex, alpha) {
    alpha = (alpha !== undefined) ? Math.max(0, Math.min(1, alpha)) : 1

    let rgb = that.decode(rgbOrHex)
    if (!rgb) return null
