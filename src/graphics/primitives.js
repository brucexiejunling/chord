//  primitives
import {objcontains, objmerge, isArray} from '../utils'

let Primitives = function (ctx, _drawStyle, _fontStyle) {
  ///MACRO:primitives-start
  let _Oval = function (x, y, w, h, style) {
    this.x = x
    this.y = y
    this.w = w
    this.h = h
  ...
  let _Rect = function (x, y, w, h, r, style) {
  ...
  let _Path = function (x1, y1, x2, y2, style) {
    // calling patterns:
    // ƒ( x1, y1, x2, y2, <style> )
  ...
  _Path.prototype = {
    draw: function (overrideStyle) {
      ...
      ctx.save()
      ctx.beginPath();
      sublines.forEach(function (lineseg) {
        ctx.moveTo(lineseg[0].x + .5, lineseg[0].y + .5);
        lineseg.forEach(function (pt, i) {
          if (i == 0) return
          ctx.lineTo(pt.x + .5, pt.y + .5);
        })
      })

      let style = Object.assign(objmerge(_drawStyle, this.style), overrideStyle)
      if (style.closed) ctx.closePath()

      if (style.fill !== undefined) {
        let fillColor = Colors.decode(style.fill, (style.alpha !== undefined) ? style.alpha : 1)
        if (fillColor) ctx.fillStyle = Colors.encode(fillColor)
        ctx.fill()
      }

      if (style.stroke !== undefined) {
        let strokeColor = Colors.decode(style.stroke, (style.alpha !== undefined) ? style.alpha : 1)
